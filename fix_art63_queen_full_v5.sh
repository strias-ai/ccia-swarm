...
echo "================================================================================"
echo "👑 DESPLEGANDO ARTEFACTO 63 v5.2 (SINTAXIS 100% CORREGIDA + REINA STRICT JSON)"
echo "================================================================================"
...
# -*- coding: utf-8 -*-
"""
CCiA Artefacto 63 - Centro de Mando y Control Tri-Enjambre + Enjambre Reina v5.2
"""
...
class TriSwarmOrchestrator:
    ...
    def parse_antispam_decision(self, raw_out):
        ...
        if "caracteres repetitivos" in low or "intento de spam" in low or "honeypot" in low or "spamming" in low:
    ...
    def run_full_pipeline(self, target_repo=None, target_issue=None):
        ...
        try:
            ...
            out_1 = self.execute_brain_turn(q1_obj, "Filtro Antispam y Relevancia", q1_prompt)
    ...
    def run_autonomous_daemon_loop(self):
        print("\n🔄 [MODO AUTÓNOMO 24/7 INICIADO] Escaneando bounties dinámicamente con Enjambre Reina v5.2...")
        while True:
            try:
...
def show_mando_menu():
    ...
    while True:
        ...
        if opt == "1":
            ...
            print("\n💳 CONFIGURACIÓN DE CARTERAS DE RECEPCIÓN")
            print(f"  1. Lightning Address : {orch.wallets.get('lightning_address')}")
            print(f"  2. EVM Address       : {orch.wallets.get('evm_address')}")
            print(f"  3. BTC Address       : {orch.wallets.get('btc_address')}")
            c = input("\n¿Deseas editar alguna cartera? (1/2/3 o Enter para salir): ").strip()
            if c == "1":
            ...
            orch.save_wallets()
            print("✅ Configuración de carteras guardada.")
            pause_terminal()
            ...
            try:
                proc = subprocess.Popen(["tail", "-n", "40", "-f", DAEMON_LOG_FILE])
                proc.wait()
            ...
            conn = sqlite3.connect(orch.db_path)
            cur = conn.cursor()
            for tbl in ["bounty_swarm_history", "bounty_vector_memory", "queen_audits", "ccia_artifact_manifests"]:
                try:
                    cur.execute(f"SELECT COUNT(*) FROM {tbl};")
                    count = cur.fetchone()[0]
                    print(f"  Tabla [{tbl:<25}]: {count} registros")
                except Exception:
                    print(f"  Tabla [{tbl:<25}]: No existe o no se puede leer")
            conn.close()
            models = orch.list_ollama_models()
            print(f"  Conexión Ollama Local: OK ({len(models)} modelos detectados)")
            pause_terminal()
...
echo "  🔍 Verificando sintaxis con py_compile..."
python3 -m py_compile "$ART63_MODULE"
echo "  ✅ Código Python compilado sin errores de sintaxis."

rm -f /tmp/ccia_bounty.lock
echo "================================================================================"
echo "✅ INSTALACIÓN INTEGRAL v5.2 COMPLETADA CON ÉXITO"
echo "================================================================================"
