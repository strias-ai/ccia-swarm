...
def build_clean_mando_code():
    return '''def show_mando_menu():
...
def patch_file(filepath):
    print(f"🛠️ Reconstrucción limpia de {filepath}...")
    with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    # Reemplazar desde def show_mando_menu hasta if __name__ o def mostrar_carteras
    mando_code = build_clean_mando_code()

    pattern = r"def show_mando_menu\(\):.*?(?=(def mostrar_carteras|if __name__|\Z))"
    new_content, count = re.subn(pattern, mando_code + "\n\n", content, flags=re.DOTALL)

    if count == 0:
        print("  ⚠️ Marcador show_mando_menu no encontrado con regex, añadiendo al final.")
        new_content = content + "\n\n" + mando_code

    # Limpieza de fragmentos huérfanos pre-menú
    new_content = re.sub(r"\n\s*elif opt == \"[^\"]+\":\s*\n\s*orch\.[^\n]+\n\s*pause_terminal\(\)", "", new_content)
...
patch_file("/home/k1/ccia_workspace/ccia_mando_63.py")
patch_file("/home/k1/ccia_workspace/modules/art_63.py")
