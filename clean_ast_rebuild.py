        ...
        if opt == "1":
            ...
            if res.returncode == 0:
                try:
                    ...
                    sel = input("\nSelecciona nº para procesar (0 para cancelar): ").strip()
...
def prune_and_rebuild(filepath):
    print(f"🛠️ Purgando código corrupto y reconstruyendo {filepath}...")
    with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
        lines = f.readlines()

    # Encontrar el prefijo válido más largo mediante AST
    last_valid_idx = 0
    for i in range(1, len(lines) + 1):
        try:
            ast.parse("".join(lines[:i]))
            last_valid_idx = i
        except SyntaxError:
            pass

    clean_head = "".join(lines[:last_valid_idx]).rstrip()
    
    # Asegurar importaciones requeridas
    required_imports = "import os, sys, sqlite3, json, subprocess, argparse\n"
    if "import os" not in clean_head:
        clean_head = required_imports + clean_head

    full_code = clean_head + "\n\n" + menu_code.strip() + main_block
...
prune_and_rebuild("/home/k1/ccia_workspace/modules/art_63.py")
prune_and_rebuild("/home/k1/ccia_workspace/ccia_mando_63.py")
